...
export function isGeminiUnknownSessionError(stdout: string, stderr: string): boolean {
  ...
  return /unknown\s+session|session\s+.*\s+not\s+found|resume\s+.*\s+not\s+found|checkpoint\s+.*\s+not\s+found|cannot\s+resume|failed\s+to\s+resume|invalid\s+session\s+(?:id(?:entifier)?|token)?|error\s+resuming\s+session/i.test(
    haystack,
  );
}
